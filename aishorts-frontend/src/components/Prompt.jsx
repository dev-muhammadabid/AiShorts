...
const useWebSocket = (url, onMessage, onError) => {
  ...
  const connect = useCallback(() => {
    ...
    client.onStompError = (frame) => {
      ...
      onError(`Connection error${retryCount.current > 0 ? ` (Retry ${retryCount.current}/${maxRetries})` : ''}`);
      if (retryCount.current < maxRetries) {
        retryCount.current++;
        setTimeout(() => client.activate(), Math.min(5000 * retryCount.current, 30000));
      } else {
...
function Prompt() {
  ...
  const handleInputSubmit = useCallback(async (userMessage) => {
    setLoading(true);
    setResponse(null);
    setError(null);
